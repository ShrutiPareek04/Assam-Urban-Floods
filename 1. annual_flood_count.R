...
# The aim of this script is to calculate the number of floods occurring in Assam.
# For this, we take inspiration from the Actuarial Climate Index (ACI).
# Days with extreme rainfall (>90th quantile) = flood
...
df3$Annual_Flood_Count <- sapply(df3$Year, function(y) { sum(df2$Monthly_Flood_Count[df2$Year == y], na.rm = TRUE) })
