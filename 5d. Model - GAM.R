# fourth model - GAM
# GLM captures linear relationships, to allow for non-linear interactions, we consider GAM
# the script also contains XAI techniques applied on GAM
# CUSTOM CODE was developed to create a GAM waterfall plot (similar to SHAP waterfall plot)
...
df_norm$Residuals <- df_norm$Flood - df_norm$GAM_Predicted
ggplot(df_norm, aes(x = GAM_Predicted, y = Residuals)) +
  geom_point(color = "purple", size = 3) +
  geom_hline(yintercept = 0, linetype = "dashed") +
  labs(title = "GAM: Residuals vs Predicted", x = "Predicted Flood Count", y = "Residuals") +
  theme_minimal()
